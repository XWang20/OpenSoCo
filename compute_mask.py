...
file_num_per_json = 100000

logger = logging.getLogger(__name__) 
def get_args():
    parser = argparse.ArgumentParser()
    group = parser.add_argument_group(title='input data')
    group.add_argument('--input_dataset', default="/mnt/sfs_turbo/caohanwen/data", type=str, help='Path to input TXT') 
    ...
    group.add_argument("--output_prefix", default="/mnt/sfs_turbo/caohanwen/data/masked_output/XLM_", type=str)
    ...
    group.add_argument('--random_seed', type=int, default=3528)
...
def train_tokenizer():
    ...
    file_dir = '/data/private/caohanwen/OpenSoCo/raw_reddit_dedup'
    files1 = [os.path.join(file_dir,filename) for filename in os.listdir(file_dir)]
    file_dir = '/data/private/caohanwen/OpenSoCo/raw_twitter_dedup'
    files2 = [os.path.join(file_dir,filename) for filename in os.listdir(file_dir)]
    files = files1 + files2
    import random
    ...
    tokenizer.save('/data/private/caohanwen/OpenSoCo/tokenizer/tokenizer_twitter_reddit.json')

def get_tokenizer():
    tokenizer = Tokenizer.from_file('/mnt/sfs_turbo/caohanwen/data/tokenizer/XLM/XLM.json')
    return tokenizer
...
def main():
    ...
    # output_prefix = "/root/bm_train_codes/masked_output/dedup_wwm"
    ...
    k = 0 #start index of masked_output file
    total_full_output = []
    total_filename = []
    for filename in filename_list: # for every imput file
        ...
        if len(total_full_output) >= file_num_per_json or filename == filename_list[-1]:
